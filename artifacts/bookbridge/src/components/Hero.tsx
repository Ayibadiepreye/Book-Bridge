...
export default function Hero() {
  ...
  return (
    <section style={{
      ...
      {/* Decorative blobs */}
      <div style={{ position: 'absolute', top: '-10%', left: '-5%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(14,165,233,.18) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '-15%', right: '-5%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(124,58,237,.14) 0%, transparent 70%)', pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto', padding: '100px 24px 60px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}
        ...
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .8, ease: 'easeOut' }}>
          <div style={{
            ...
            <span style={{ color: '#7DD3FC', fontSize: '.82rem', fontWeight: 600, letterSpacing: '.5px', textTransform: 'uppercase' }}>Supporting SDG 4 · Quality Education</span>
          </div>
          ...
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", color: '#ffffff', lineHeight: 1.05, margin: 0 }}>
            ...
            <span style={{ display: 'block', fontSize: 'clamp(3rem, 5.5vw, 4.8rem)', fontWeight: 700, background: 'linear-gradient(90deg, #38BDF8, #FF0090)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', fontStyle: 'italic' }}>Dreams</span>
          ...
          <p style={{ color: 'rgba(255,255,255,.75)', fontSize: '1.05rem', lineHeight: 1.75, margin: '24px 0 36px', maxWidth: 480 }}>
            Project BookBridge empowers underserved junior secondary students across Nigeria with donated books, quality learning materials — one student at a time.
          </p>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginBottom: 48 }}>
            ...
            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 14px 32px rgba(14,165,233,.55)'; }}
            onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(14,165,233,.45)'; }}
            >
          ...
          <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
            {[
              ...
              { value: '350+', label: 'Students Reached' },
              { value: '12+', label: 'Communities' },
            ].map(s => (
              <div key={s.label}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.2rem', fontWeight: 700, color: '#38BDF8', lineHeight: 1 }}>{s.value}</div>
        ...
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .8, ease: 'easeOut', delay: .15 }}
          style={{ position: 'relative', borderRadius: 24, overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,.4)', aspectRatio: '3/4', maxHeight: 580 }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px 20px' }}>
            <p style={{ color: 'rgba(255,255,255,.9)', fontSize: '.82rem', margin: 0, fontStyle: 'italic' }}>{heroCarousel[currentIdx]?.caption}</p>
          </div>
          ...
          <div style={{ position: 'absolute', bottom: 44, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: 6 }}>
            {heroCarousel.map((_, i) => (
              <button key={i} onClick={() => emblaApi?.scrollTo(i)} style={{
                ...
                background: i === currentIdx ? '#38BDF8' : 'rgba(255,255,255,.4)',
                transition: 'all .3s',
              }} />
            ))}
          </div>
